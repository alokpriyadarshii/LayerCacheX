...
import type { LayerStore } from './layer_store.js'
import { CacheStack } from './cache/cache_stack.js'
import type { CacheProvider } from './types/provider.js'
import { LayerCacheXOptions } from './layer_cache_x_options.js'
import type {
  CacheEvents,
  RawLayerCacheXOptions,
  LayerCacheXPlugin,
  ClearOptions,
...
export class LayerCacheX<KnownCaches extends Record<string, LayerStore>> implements CacheProvider {
  /**
  ...
  /**
   * LayerCacheX options instance
   */
  #options: LayerCacheXOptions

  constructor(
    config: RawLayerCacheXOptions & {
      default: keyof KnownCaches
      stores: KnownCaches
      plugins?: LayerCacheXPlugin[]
    },
  ) {
    this.#stores = config.stores
    this.#defaultStoreName = config.default

    this.#options = new LayerCacheXOptions(config)
    this.#options.logger.trace('layercachex initialized')
  ...
  #createProvider(cacheName: string, store: LayerStore): CacheProvider {

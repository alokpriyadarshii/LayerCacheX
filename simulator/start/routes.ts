...
import type { CreateBusDriverResult, CreateDriverResult, L2CacheDriver } from 'layercachex/types'
...
const nodes: Map<string, { layerCacheX: LayerCacheX<any>; bus: ChaosBus; l2: ChaosCache<L2CacheDriver> }> =
  new Map()

const trueCache = new LayerCacheX({
  default: 'default',
  prefix: '',
  grace: '2h',
  stores: {
    default: layerstore().useL2Layer(
...
router.get('/', async ({ inertia }) => {
  const results = await Promise.all(
    nodes.entries().map(async ([key, cache]) => {
      return {
        name: key,
        result: await cache.layerCacheX.get({ key: 'value', defaultValue: 0 }),
        busId: cache.bus.id,
      }
    }),
  )
...
router.post('/set', async ({ request, response }) => {
  ...
  const currentValue = await cache!.layerCacheX.get({ key: 'value', defaultValue: 0 })
  await cache?.layerCacheX.set({ key: 'value', value: currentValue + 1 })
...
router.post('/delete', async ({ request, response }) => {
  ...
  const cache = nodes.get(cacheName)
  await cache?.layerCacheX.delete({ key: 'value' })
...
router.post('/state', async ({ request, response }) => {
  ...
  if (!isUndefined(payload.bus)) {
    state.bus = payload.bus
  }

  if (!isUndefined(payload.l2)) {
    state.l2 = payload.l2
  }

  nodes.forEach((cache) => {
    if (!isUndefined(payload.bus) && !payload.bus) {
      cache.bus.alwaysThrow()
    } else {
      cache.bus.neverThrow()
    }

    if (!isUndefined(payload.l2) && !payload.l2) {
